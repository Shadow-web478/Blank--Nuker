...
def main():
    ...
    async def delete_roles(ctx):
        try:
            guild = ctx.guild
            roles_deleted = 0  # Counter for deleted roles
            for role in guild.roles:
                if role.name != "@everyone":
                    await role.delete()
                    roles_deleted += 1  # Increment counter after deleting each role
            if roles_deleted > 0:
                await ctx.send(f'Deleted {roles_deleted} roles (except @everyone).')
            else:
                await ctx.send('No roles were deleted.')
        except discord.Forbidden:
            await ctx.send("I don't have permission to delete roles.")
        except discord.HTTPException:
            await ctx.send("Failed to delete the roles due to a network error.")
